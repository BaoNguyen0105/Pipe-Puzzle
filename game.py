...
class MapGenerator:
    @classmethod
    def default_map_5(cls) -> Map:
        map = Map.load_grid([
            [
                PipeGenerator.sink(), 
                PipeGenerator.straight(), 
                PipeGenerator.t(), 
                PipeGenerator.straight(), 
                PipeGenerator.sink()
            ],
            [
                PipeGenerator.corner(), 
                PipeGenerator.corner(), 
                PipeGenerator.straight(), 
                PipeGenerator.corner(), 
                PipeGenerator.corner()
            ...
            [
                PipeGenerator.sink(), 
                PipeGenerator.straight(), 
                PipeGenerator.corner(is_source=True), # Nguồn nằm ở chính giữa
                PipeGenerator.t(), 
                PipeGenerator.sink()
            ],
            [
    ...
    def default_map_4(cls) -> Map:
        map=Map.load_grid([
                ...
                [PipeGenerator.sink(), PipeGenerator.t(), PipeGenerator.corner(is_source=True), PipeGenerator.straight()],
    ...
    @classmethod
    def generate_solvable_map(cls, width: int, height: int) -> Map:
        # 1. Khởi tạo lưới các hướng kết nối (mảng 2 chiều chứa set các Direction)
        grid_dirs = [[set() for _ in range(width)] for _ in range(height)]
        
        # 2. Tạo cây bao trùm ngẫu nhiên (Randomized DFS) để đảm bảo mọi ô đều thông nhau
        visited = set()
        stack = [(randint(0, width-1), randint(0, height-1))]
        visited.add(stack[0])
        ...
        while stack:
            ...
            for d, dx, dy, opp in directions:
                ...
                if 0 <= nx < width and 0 <= ny < height and (nx, ny) not in visited:
                    neighbors.append((d, nx, ny, opp))
            
            if neighbors:
                d, nx, ny, opp = choice(neighbors)
                grid_dirs[cy][cx].add(d)     # Ô hiện tại nối sang ô mới
                grid_dirs[ny][nx].add(opp)   # Ô mới nối ngược lại ô hiện tại
                visited.add((nx, ny))
                stack.append((nx, ny))
            else:
                stack.pop()

        # 3. Chuyển đổi lưới hướng thành các đối tượng Pipe cụ thể
        pipe_grid = []
        for _ in range(100):
            x=randint(0,width-1)
            y=randint(0,height-1)
            if len(grid_dirs[y][x])!=1:
                source_pos=(x,y)
        ...
        for y in range(height):
            ...
            for x in range(width):
                dirs = grid_dirs[y][x]
                is_src = (x, y) == source_pos
                
                # Xác định loại ống dựa trên số lượng và hướng kết nối
                pipe = cls._create_pipe_by_dirs(dirs, is_src)
                
                row.append(pipe)
            pipe_grid.append(row)

        # 4. Load vào Map và cập nhật dòng chảy
        generated_map = Map.load_grid(pipe_grid)
        generated_map._update_water_flow()
        return generated_map

    @staticmethod
    def _create_pipe_by_dirs(dirs, is_source):
        count = len(dirs)
        # Logic ánh xạ từ kết nối sang loại ống (giả định các hàm của PipeGenerator)
        if count <= 1:
            return PipeGenerator.sink()
        elif count == 2:
            # Kiểm tra xem là ống thẳng hay ống góc
            d_list = list(dirs)
            if (Direction.UP in d_list and Direction.DOWN in d_list) or \
               (Direction.LEFT in d_list and Direction.RIGHT in d_list):
                return PipeGenerator.straight(is_source=is_source)
            else:
                return PipeGenerator.corner(is_source=is_source)
        elif count == 3:
            return PipeGenerator.t(is_source=is_source)
        else:
            return PipeGenerator.cross(is_source=is_source)
